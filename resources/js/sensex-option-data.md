...
// ══════════════════════════════════════════════════════════
// ✅ FIX: Client-side rate limiter — prevents AI API overuse
// Limits: 10 AI calls per session, 3 chat msgs per minute
// ══════════════════════════════════════════════════════════
const RateLimit = (() => {
    const CHAT_MAX_PER_MIN   = 8;   // max chat messages per minute (side + modal combined)
    const ANALYZE_MAX_SESSION = 15;  // max AI analyze calls per page session

    const state = {
        chatTimestamps: [],   // rolling 60s window
        analyzeCount: 0,
    };

    function _pruneChat() {
        const now = Date.now();
        state.chatTimestamps = state.chatTimestamps.filter(t => now - t < 60000);
    }

    return {
        canChat() {
            _pruneChat();
            return state.chatTimestamps.length < CHAT_MAX_PER_MIN;
        },
        recordChat() {
            state.chatTimestamps.push(Date.now());
            _pruneChat();
            _updateChatUI();
        },
        canAnalyze() {
            return state.analyzeCount < ANALYZE_MAX_SESSION;
        },
        recordAnalyze() {
            state.analyzeCount++;
        },
        chatRemaining() {
            _pruneChat();
            return Math.max(0, CHAT_MAX_PER_MIN - state.chatTimestamps.length);
        },
        chatResetSecs() {
            if (!state.chatTimestamps.length) return 0;
            const oldest = Math.min(...state.chatTimestamps);
            return Math.max(0, Math.ceil((oldest + 60000 - Date.now()) / 1000));
        },
        analyzeRemaining() {
            return Math.max(0, ANALYZE_MAX_SESSION - state.analyzeCount);
        },
    };

    function _updateChatUI() {
        // update side panel badge
        const badge = document.getElementById('chatRateBadge');
        const left  = document.getElementById('chatRateLeft');
        const reset = document.getElementById('chatRateReset');
        if (badge) badge.classList.remove('hidden');
        if (badge) badge.style.display = 'flex';
        if (left)  left.textContent = RateLimit.chatRemaining() + ' / ' + CHAT_MAX_PER_MIN + ' remaining';
        if (reset && RateLimit.chatResetSecs() > 0) reset.textContent = 'resets in ' + RateLimit.chatResetSecs() + 's';
        // modal badge
        const mb = document.getElementById('modalRateBadge');
        const ml = document.getElementById('modalRateLeft');
        if (mb) mb.style.display = 'flex';
        if (ml) ml.textContent = RateLimit.chatRemaining() + ' left';
    }
})();
...
// ✅ FIX: openSensexChart now guards against empty/null tokens
function openSensexChart(token, label, exchange, otherToken, strike, side) {
    ...
    if (!token || String(token).trim() === '' || token === 'null' || token === 'undefined') {
        console.error('openSensexChart: invalid token', token);
        alert('⚠️ Is strike ka token available nahi hai. Scrip cache refresh karo.');
        return;
    }
    G.token    = String(token).trim();
    G.exchange = exchange || 'BFO';
    G.interval = 'FIVE_MINUTE';
    G.label    = label;
    G.strike   = strike;
    G.side     = side;
    ...
    document.getElementById('sxMTyping')?.remove();
    document.getElementById('sxSTyping')?.remove();
    document.getElementById('sensexModalTitle').textContent = label;
    document.getElementById('sensexChartModal').style.display = 'flex';
    _sxResetAIPanel();
    document.getElementById('sxModalChatMsgs').innerHTML = '';
    _addModalBubble(`<b>${label}</b> chart open hai — poochho kuch bhi! 📊`, 'ai');
    _styleIv('FIVE_MINUTE'); _styleTick(2000); _styleChartBtn('candlestick');
    _loadChart();
}
...
// ✅ FIX: _loadChart guards token before fetch
async function _loadChart(){
    if(!G.token){
        document.getElementById('sxChartLoader').style.display='none';
        document.getElementById('sxChartError').style.display='flex';
        document.getElementById('sxErrMsg').textContent='Token missing — chart load nahi ho sakta.';
        return;
    }
    stopTickFeed();
    ...
    try{
        const res=await fetch(`${CANDLE_URL}?token=${encodeURIComponent(G.token)}&exchange=${encodeURIComponent(G.exchange)}&interval=${G.interval}&_t=${Date.now()}`,{headers:{'X-Requested-With':'XMLHttpRequest'}});
        if(!res.ok)throw new Error(`HTTP ${res.status} — server error`);
        const json=await res.json();
        // ✅ FIX: show server message in error box, not generic text
        if(!json.success)throw new Error(json.message||'Candle data nahi mila.');
        if(!json.data||!json.data.length)throw new Error('Data empty hai — market band ho sakta hai ya token galat hai.');
        G.rawCandles=json.data;_buildChart(json.data);
    }catch(e){
        loader.style.display='none';errBox.style.display='flex';document.getElementById('sxErrMsg').textContent=e.message;document.getElementById('sxCandleCount').textContent='Error';
    }
}
...
function _istFmt(ts){
    ...
    return{dd:String(ist.getDate()).padStart(2,'0'),mon:['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][ist.getMonth()],hh:String(ist.getHours()).padStart(2,'0'),mm:String(ist.getMinutes()).padStart(2,'0'),yr:ist.getFullYear()};
}
...
function _buildChart(raw){
    ...
    G.chart=LightweightCharts.createChart(container,{width:container.clientWidth,height:container.clientHeight,layout:{background:{color:'#ffffff'},textColor:'#334155'},grid:{vertLines:{color:'#f1f5f9'},horzLines:{color:'#f1f5f9'}},crosshair:{mode:LightweightCharts.CrosshairMode.Normal},timeScale:{borderColor:'#e2e8f0',timeVisible:true,secondsVisible:false,tickMarkFormatter:(ts,type)=>{const f=_istFmt(ts);return type<=2?`${f.dd} ${f.mon}`:`${f.hh}:${f.mm}`;}},rightPriceScale:{borderColor:'#e2e8f0'},localization:{timeFormatter:(ts)=>{const f=_istFmt(ts);return `${f.dd} ${f.mon} ${f.yr}  ${f.hh}:${f.mm} IST`;}}});
    ...
    const candles=raw.map(c=>{let ts;const r0=c[0];if(typeof r0==='number'){ts=r0;}else{let s=String(r0);ts=Math.floor(new Date(s.includes('+')||s.includes('Z')?s:s.replace(' ','T')+'+05:30').getTime()/1000);}return{time:ts,open:parseFloat(c[1]),high:parseFloat(c[2]),low:parseFloat(c[3]),close:parseFloat(c[4])};}).filter(c=>c.time>0&&c.open>0).sort((a,b)=>a.time-b.time);
    ...
    G.chart.subscribeCrosshairMove(param=>{if(!param.time||!param.seriesData)return;const d=param.seriesData.get(G.series);if(!d)return;const f=_istFmt(param.time);document.getElementById('sxOhlcBar').classList.remove('hidden');document.getElementById('sxOhlcLabel').textContent=`${f.dd} ${f.mon}  ${f.hh}:${f.mm} IST`;const o=d.open??d.value??0,h=d.high??d.value??0,l=d.low??d.value??0,cl=d.close??d.value??0;document.getElementById('sxOVal').textContent=o.toFixed(2);document.getElementById('sxHVal').textContent=h.toFixed(2);document.getElementById('sxLVal').textContent=l.toFixed(2);document.getElementById('sxCVal').textContent=cl.toFixed(2);const chg=o?(((cl-o)/o)*100).toFixed(2):0;const tag=document.getElementById('sxChangeTag');tag.textContent=(chg>=0?'▲ +':'▼ ')+chg+'%';tag.style.color=chg>=0?'#22c55e':'#ef4444';});
...
// ✅ FIX: AI analyze guarded by rate limiter
async function runSensexAiAnalyze(){
    if(G.aiAnalyzing)return;
    // ✅ Rate limit check
    if(!RateLimit.canAnalyze()){
        const wp=document.getElementById('sxAiWaiting')?.querySelector('p');
        _sxShow('sxAiWaiting');_sxHide('sxAiSkeleton');_sxHide('sxAiVerdictArea');
        if(wp)wp.innerHTML=`<span style="color:#dc2626;">⚠️ AI analyze limit reach ho gayi (session mein).</span><br><small style="color:#64748b;">Page refresh karo.</small>`;
        return;
    }
    G.aiAnalyzing=true;RateLimit.recordAnalyze();
    _sxHide('sxAiWaiting');_sxHide('sxAiVerdictArea');_sxShow('sxAiSkeleton');
    ...
    G.aiAnalyzing=false;if(btn)Object.assign(btn.style,{opacity:'1',cursor:'pointer'});if(txt)txt.textContent='🔄 Re-Analyze ('+RateLimit.analyzeRemaining()+' left)';
}
...
// ✅ FIX: sendChat with rate limiter
async function sendChat(){
    const inp=document.getElementById('chatInput');const msg=inp.value.trim();if(!msg)return;
    // ✅ Rate limit check
    if(!RateLimit.canChat()){
        _addBubble(`⏳ Thoda ruko — ${RateLimit.chatResetSecs()}s baad fir try karo (limit: ${RateLimit.chatRemaining()} msgs/min).`,'err','chatMessages');
        inp.value='';return;
    }
    if(G.sideChatTyping)return; // ✅ FIX: prevent double-send while waiting
    G.sideChatTyping=true;RateLimit.recordChat();
    inp.value='';
    const sendBtn=document.getElementById('chatSendBtn');if(sendBtn)sendBtn.disabled=true;
    _addBubble(msg,'user','chatMessages');G.chatHistory.push({role:'user',content:msg});const tid=_addTyping('chatMessages');
    ...
    catch(e){_removeTyping(tid);_addBubble('❌ Error: '+e.message,'err','chatMessages');}
    finally{G.sideChatTyping=false;if(sendBtn)sendBtn.disabled=false;}
}
...
// ✅ FIX: sendModalChat with rate limiter + disabled state
async function sendModalChat(){
    const inp=document.getElementById('sxModalChatInput');const msg=inp.value.trim();if(!msg)return;
    // ✅ Rate limit check
    if(!RateLimit.canChat()){
        _addModalBubble(`⏳ Thoda ruko — ${RateLimit.chatResetSecs()}s baad fir try karo.`,'err');
        inp.value='';return;
    }
    if(G.modalChatTyping)return; // ✅ FIX: prevent race condition
    G.modalChatTyping=true;RateLimit.recordChat();
    inp.value='';
    const dot=document.getElementById('sxChatStatusDot');
    const sendBtn=document.getElementById('sxModalSendBtn');
    ...
    finally{G.modalChatTyping=false;if(dot){dot.textContent='● Ready';dot.style.color='#16a34a';}if(sendBtn)sendBtn.disabled=false;}
}
